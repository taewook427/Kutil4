...
class toolbox:
    ...
    def __init__(self):
        ...
        self.dll = 0 # module native assist dll
        self.assist = False # native assist check
    ...
    def mkkey(self, salt, pw, kf): # bytes, bytes, bytes
        return scrypt.hash(kf + pw + pw + kf + pw, salt, 16384, 8, 1, 48) # bytes 48B

    # generate key storage 256B <native assist>
    def svkey(self, salt, pw, kf): # bytes, bytes, bytes
        return scrypt.hash(pw + pw + kf + kf + pw, salt, 524288, 8, 1, 256) # bytes 256B
    ...
    def enwhole(self, pw, kf, hint, data): # bytes, bytes, bytes, path str / bytes
        if type(data) == bytes:
            ...
            for i in range(0, num0):
                ...
                temp[count] = p.apply_async( self.enshort, (key[count], iv[count], tempdata) )
                if count == 31:
                    for j in range(0,32):
                        temp[j] = temp[j].get()
                        iv[j] = temp[j][-16:]
            ...
            out.append( self.enshort(key[count], iv[count], tempdata) )
            p.close()
            p.join()

            return header + b''.join(out) # bytes
            ...
            with open(tgt,'wb') as f:
                with open(path,'rb') as t:
                    ...
                    for i in range(0, num0):
                        ...
                        order[count] = p.apply_async( self.enshort, (key[count], iv[count], tempdata) )
                        if count == 31:
                            for j in range(0,32):
                                write[j] = order[j].get()
                                iv[j] = write[j][-16:]
                    ...
                    f.write( self.enshort(key[count], iv[count], tempdata) )
            p.close()
            p.join()
    ...
    def dewhole(self, pw, kf, data): # bytes, bytes, bytes, path str / bytes
        if type(data) == bytes:
            ...
            if self.svkey(salt, pw, kf) == pwh:
                ...
                for i in range(0,num0):
                    tempdata = data[131072 * i : 131072 * i + 131072] # 128kb
                    temp[count] = p.apply_async( self.deshort, (key[count], iv[count], tempdata) )
                ...
                tempdata = data[131072 * num0:]
                out.append( unpad( self.deshort(key[count], iv[count], tempdata) ) )
                p.close()
                p.join()
                ...
                with open(path,'rb') as f:
                    with open(tgt,'wb') as t:
                        ...
                        for i in range(0, num0):
                            ...
                            order[count] = p.apply_async( self.deshort, (key[count], iv[count], tempdata) )
                            iv[count] = tempdata[-16:]
                            if count == 31:
                                for j in range(0,32):
                                    write[j] = order[j].get()
                        ...
                        tempbyte = self.deshort(key[count], iv[count], tempdata)
